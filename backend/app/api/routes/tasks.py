...
def create_task(task: TaskCreate, sprint_id: int, session: Session = Depends(get_session)):
...
def update_task(task_id: int, task_update: TaskUpdate, session: Session = Depends(get_session)):
    return TaskService.update_task(session, task_id, **task_update.dict(exclude_unset=True))

@router.get("/sprint/{sprint_id}")
def get_sprint_tasks(sprint_id: int, session: Session = Depends(get_session)):
